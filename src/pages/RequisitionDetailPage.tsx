...
export default function RequisitionDetailPage() {
  ...
  // Envoyer un brouillon (draft → pending sans WhatsApp)
  const handleSendDraft = useCallback(async () => {
    if (!id || !profile) return
    setActionLoading(true)
    await updateStatus(id, 'pending', profile.id)
    setActionLoading(false)
    if (id) fetchRequisitionDetail(id)
  }, [id, profile, updateStatus, fetchRequisitionDetail])

  // Envoyer un brouillon via WhatsApp (draft → pending + ouvrir WhatsApp)
  const handleSendDraftWhatsApp = useCallback(async () => {
    if (!id || !profile) return
    setActionLoading(true)
    await updateStatus(id, 'pending', profile.id)
    setActionLoading(false)
    ...
    // Recharger
    if (id) fetchRequisitionDetail(id)
  }, [id, profile, req, whatsappDestNumber, updateStatus, fetchRequisitionDetail])
            ...
            <div className="mt-6 space-y-3 pb-8">
              ...
              {isEditable && (
                <button onClick={handleStartEdit}
                  ...
                  {isDraft ? 'Reprendre le brouillon' : 'Modifier la réquisition'}
                </button>
              )}
              ...
              {isDraft && (
                <>
                  <button onClick={handleSendDraft} disabled={actionLoading}
                    className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-blue-600 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-blue-700 disabled:opacity-60">
                  ...
                  {hasWhatsApp && (
                    <button onClick={handleSendDraftWhatsApp} disabled={actionLoading}
                      className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-green-600 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-green-700 disabled:opacity-60">
                      {actionLoading ? <LoadingSpinner size="sm" /> : (
                        <>
