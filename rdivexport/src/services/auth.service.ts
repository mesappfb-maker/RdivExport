...
function mapRowToProfile(
  ...
  return {
    ...
    pharmacy_id: (row as any).pharmacy_id ?? undefined,
    pharmacy: row.pharmacies
      ? {
          id: row.pharmacies.id,
          name: row.pharmacies.name,
          code: row.pharmacies.code,
          address: (row.pharmacies as any).address ?? null,
          phone: (row.pharmacies as any).phone ?? null,
          whatsapp_number: (row.pharmacies as any).whatsapp_number ?? null,
          email: (row.pharmacies as any).email ?? null,
          is_active: (row.pharmacies as any).is_active !== false,
